...
func (p *TimeParser) ParseValue(body []byte) (map[string]interface{}, error) {
	...
	for {
		...
		if match {
			...
			sysTime := value
			values["user time "+timeUnit] = float64(userTime)
			values["sys time "+timeUnit] = float64(sysTime)
			values["real time "+timeUnit] = float64(realTime)
			break
		}
	}
	return values, nil
...
func (p *TimeParser) GetPerformanceValue(values map[string]interface{}) (string, float64) {
	performanceKey := "real time " + timeUnit
